...
class PlayState extends MusicBeatState
{
	...
	// ROOM VARS
	var camCopy:FlxSprite;
	var room:FlxSprite;
	var clockface:FlxSprite;
	var clockminute:FlxSprite;
	var clocksecond:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'room':
				room = new FlxSprite(0,-700).loadGraphic(Paths.image('roombg'));
				add(room);
				clockface = new FlxSprite(2200,150).loadGraphic(Paths.image('clockstuffs/CLOCK'));
				add(clockface);
				clockminute = new FlxSprite(2200,150).loadGraphic(Paths.image('clockstuffs/minutes'));
				add(clockminute);
				clockminute.origin.y = 277/2 - 5;
				clocksecond = new FlxSprite(2200,150).loadGraphic(Paths.image('clockstuffs/seconds'));
				add(clocksecond);
				clocksecond.origin.y = 277/2 - 5;

				camCopy = new FlxSprite(1297,72);
				camCopy.makeGraphic(Std.int(FlxG.width/ClientPrefs.getTVResolution()), Std.int(FlxG.height/ClientPrefs.getTVResolution()));
				add(camCopy);
				camCopy.scale.set(ClientPrefs.getTVResolution()/ 1.6666666666666666666666666666667, ClientPrefs.getTVResolution()/1.6744186046511627906976744186047);
				camCopy.antialiasing = true;
				camCopy.updateHitbox();
		...
		scoreTxt.setFormat(Paths.font("lyricfont.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.SHADOW, FlxColor.BLACK);
		...
		lyricsTxt.setFormat(Paths.font("lyricfont.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.SHADOW, FlxColor.BLACK);
		...
		var doPush:Bool = false;
		var luaFile:String = 'data/' + Paths.formatToSongPath(SONG.song) + '/sex.lua';
		luaFile = Paths.getPreloadPath(luaFile);
...
// SONG SPECIFIC SCRIPTS
		...
		var doPush:Bool = false;
		var luaFile:String = 'data/' + Paths.formatToSongPath(SONG.song) + '/sex2.lua';
		luaFile = Paths.getPreloadPath(luaFile);
		if (OpenFlAssets.exists(luaFile))
		{
			doPush = true;
	...
	inline function cacheCountdown()
	{
	...
	public inline function updateScore(miss:Bool = false)
	{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL - (198.5 /2) : STRUM_X - (198.5 /2), strumLine.y - (212.5/2), i, player);
			babyArrow.downScroll = ClientPrefs.downScroll;
			if (!isStoryMode && !skipArrowStartTween)
			{
				//babyArrow.y -= 10;
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {/*y: babyArrow.y + 10,*/ alpha: targetAlpha}, 1 + (i/8), {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
			}
			else
			{
	...
	override public function update(elapsed:Float)
	{
		...
		{
			case 'room':
				clockminute.angle = (Conductor.songPosition / songLength) * 360;
				clocksecond.angle = clockminute.angle * 60;
				if (curFrame % 40 <= 20) {
					var cameraOriginal = camGame;
					...
					transformMatrix.scale(cameraOriginal.scaleX/ClientPrefs.getTVResolution(), cameraOriginal.scaleY/ClientPrefs.getTVResolution());
					camCopy.pixels.draw(cameraOriginal.canvas, transformMatrix);
				}
		}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				FlxTween.tween(this, {playbackRate: 0.0001}, 0.2, {ease: FlxEase.sineIn, onComplete: function(twn) {
					vocals.stop();
					FlxG.sound.music.stop();
					lyricsTxt.text = '⏮';
